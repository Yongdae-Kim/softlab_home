...
class DataStrReportController {

	final String SUBJECT_NAME = Subject.dataStr.value
	final String BOARD_NAME = "data_str_board"
	...
	def index(Integer max) {
		...
		if(params.search){
			...
			reportInstanceList = DataStrReport.findAllByStdNumLike("%${keyword}%", params)
			reportInstanceCount = DataStrReport.countByStdNumLike("%${keyword}%")
		}else{
			// ###################### 최초 레포트 목록 불러오기 ######################
			reportInstanceList = DataStrReport.list(params)
			reportInstanceCount = DataStrReport.count()
		}
	...
	def login(){
		...
		if(confirmUser){
			// ###################### 레포트 게시판 유저확인 체크 ######################
			if(confirmUser.userRole == UserRole.dataStrUser || confirmUser.userRole == UserRole.admin){
	...
	@Transactional
	def save(DataStrReport reportInstance) {
	...
	def edit(DataStrReport reportInstance) {
	...
	@Transactional
	def update(DataStrReport reportInstance) {
		// ###################### 등록 시 입력한 비밀번호 확인 여부 체크 ######################
		if(reportInstance.password == params.confirmPw){
			if (reportInstance) {
				// ###################### 업로드 파일 존재여부 체크 ######################
	...
	@Transactional
	def delete(DataStrReport reportInstance) {
