...
class SoftEngReportController {

	final String SUBJECT_NAME = Subject.softEng.value
	final String BOARD_NAME = "soft_eng_board"
	...
	def index(Integer max) {
		...
		if(params.search){
			...
			reportInstanceList = SoftEngReport.findAllByStdNumLike("%${keyword}%", params)
			reportInstanceCount = SoftEngReport.countByStdNumLike("%${keyword}%")
		}else{
			// ###################### 최초 레포트 목록 불러오기 ######################
			reportInstanceList = SoftEngReport.list(params)
			reportInstanceCount = SoftEngReport.count()
		}
	...
	def show(SoftEngReport reportInstance) {
	...
	def login(){
		...
		if(confirmUser){
			// ###################### 레포트 게시판 유저확인 체크 ######################
			if(confirmUser.userRole == UserRole.softEngUser || confirmUser.userRole == UserRole.admin){
	...
	@Transactional
	def save(SoftEngReport reportInstance) {
	...
	def edit(SoftEngReport reportInstance) {
	...
	@Transactional
	def update(SoftEngReport reportInstance) {
		if(reportInstance.password == params.confirmPw){
			if (reportInstance) {
				// ###################### 업로드 파일 존재여부 체크 ######################
				def inputFile = request.getFile('upload_file')
	...
	@Transactional
	def delete(SoftEngReport reportInstance) {
