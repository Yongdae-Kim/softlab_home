...
class CsharpReportController {

	final String SUBJECT_NAME = Subject.csharp.value
	...
	def index(Integer max) {
		...
		if(params.search){
			...
			reportInstanceList = CsharpReport.findAllByStdNumLike("%${keyword}%", params)
			reportInstanceCount = CsharpReport.countByStdNumLike("%${keyword}%")
		}else{
			// ###################### 최초 레포트 목록 불러오기 ######################
			reportInstanceList = CsharpReport.list(params)
			reportInstanceCount = CsharpReport.count()
		}
	...
	def show(CsharpReport reportInstance) {
	...
	def login(){
		...
		if(confirmUser){
			// ###################### 레포트 게시판 유저확인 체크 ######################
			if(confirmUser.userRole == UserRole.csharpUser || confirmUser.userRole == UserRole.admin){
	...
	@Transactional
	def save(CsharpReport reportInstance) {
	...
	def edit(CsharpReport reportInstance) {
	...
	@Transactional
	def update(CsharpReport reportInstance) {
		// ###################### 등록 시 입력한 비밀번호 확인 여부 체크 ######################
		if(reportInstance.password == params.confirmPw){
			if (reportInstance) {
				// ###################### 업로드 파일 존재여부 체크 ######################
	...
	@Transactional
	def delete(CsharpReport reportInstance) {
