...
class JavaReportController {

	final String SUBJECT_NAME = Subject.java.value
	final String BOARD_NAME = "java_board"
	...
	def index(Integer max) {
		...
		if(params.search){
			...
			reportInstanceList = JavaReport.findAllByStdNumLike("%${keyword}%", params)
			reportInstanceCount = JavaReport.countByStdNumLike("%${keyword}%")
		}else{
			// ###################### 최초 레포트 목록 불러오기 ######################
			reportInstanceList = JavaReport.list(params)
			reportInstanceCount = JavaReport.count()
		}
	...
	def login(){
		...
		def viewName = "/report/login"
		def modelMap = [subjectName : SUBJECT_NAME, msg : NO_REGISTED_USER_MSG]

		// ###################### 유저확인 체크 ######################
		def confirmUser = User.findByUserIdAndUserPw(params.userId, params.userPw)
		if(confirmUser){
			// ###################### 레포트 게시판 유저확인 체크 ######################
			if(confirmUser.userRole == UserRole.javaUser || confirmUser.userRole == UserRole.admin){
				viewName = "/report/create"
				modelMap = [subjectName : SUBJECT_NAME]
			}
			return render(view: viewName, model:modelMap)
		}else{
			return render(view: viewName, model:modelMap)
		}
	}
	...
	@Transactional
	def save(JavaReport reportInstance) {
	...
	def edit(JavaReport reportInstance) {
